...
- **多模型支持**：现已原生兼容并支持 OpenAI、Google Gemini 以及 Anthropic (Claude) 系列模型。
- **流式输出优化**：重构了底层的 SSE (Server-Sent Events) 数据流解析引擎，修复了数据分块截断导致的 JSON 解析报错问题。
- **配置界面升级**：优化了 Zotero 7 环境下的首选项菜单兼容性，支持无缝切换不同 API 提供商并自动补全请求后缀。
- **清理逻辑优化**：重新设计了插件右键菜单的注销逻辑，杜绝了多次重载插件或重复安装产生的幽灵菜单（重复菜单）问题。
- **平台兼容**：在 Anthropic 接口中增加了对阿里云 DashScope（如 `qwen3.7-plus`）的深度兼容（内置 `thinking: disabled`）。
