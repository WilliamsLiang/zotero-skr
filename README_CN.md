...
- **全平台多版本支持**：正式全面兼容 Zotero 7、Zotero 8 和 Zotero 9，且 Windows 和 macOS 彻底统一为一个跨平台安装包。
- **现代化 UI 重构**：全面重构了 `user-paper` 和 `user-tag` 交互界面，采用全新的 "Serif-Academic" 学术看板设计语言。
- **分类树状体系**：在标签页底层重构了与系统完全一致的左侧树状结构，并带有智能的层级序号（如 `1.1.2`），使其完全独立于标签的排序方式。
- **智能参考文献引注**：新增单篇论文多格式引注切换功能，并实现了首选项中引注默认名称的中英双语动态识别适配。
- **Markdown 渲染引擎与自动保存**：新增了内置的 Markdown 渲染解析支持，使得生成的辅助阅读笔记排版更美观，且“SKR 辅助阅读分析”功能现在会自动将生成的成果永久保存为 Zotero 子笔记。
- **大模型设置修复**：修复了初次打开首选项页面时，不能瞬间动态组合展示最终 LLM API 请求地址的 Bug。
